...
(``metadata.json``: per-video ``saw_cut_cm`` and ``date``), then draws one line:
...
Usage:
    python compare_collapse.py                    # defaults: pst_results, metadata.json
...
def _smoothed_profile(x, y, collapsed, x0, x1, bin_m, smooth_win, front_trim_bins=1):
    ...
    The first `front_trim_bins` populated bins are dropped: the markers right at
    the saw cut are disturbed by the blade kerf (saw thickness), so their collapse
    ...
    if front_trim_bins > 0 and centers.size > front_trim_bins:
        centers, vals = centers[front_trim_bins:], vals[front_trim_bins:]   # drop saw-kerf bins
...
def run(args):
    ...
    for name, m in reg.items():
        ...
        # Far end can include off-column markers the in_column test didn't trim;
        # an optional registry "usable_extent_cm" caps the line at the real slab.
        usable_m = float(m.get("usable_extent_cm", 0)) / 100.0 or col_m
        dt_h = (datetime.fromisoformat(m["date"]) - t0).total_seconds() / 3600.0
        keep = df.in_column & ~df.bad_track & (df.along_column_m >= saw_m) \
            & (df.along_column_m <= usable_m)
        cx, cy = _smoothed_profile(df.along_column_m[keep].to_numpy(),
                                   ...
                                   saw_m, usable_m, args.bin_m, args.smooth_win,
                                   args.front_trim_bins)
    ...
    for it in items:
        ...
        # Legend keys on time (not the video name); saw cut is also where the line starts.
        label = (f"{_fmt_dt(it['dt_h'])}  ·  cut {it['saw_m'] * 100:.0f} cm"
                 + ("  (pre-slope reference)" if is_ref else ""))
    ...
    ax.grid(alpha=0.3)
    ax.legend(fontsize=8, loc="upper right", title="time vs slope collapse")
    fig.tight_layout()
...
def build_parser():
    ...
    p.add_argument("--front-trim-bins", type=int, default=1,
                   help="Drop this many bins right after the saw cut (saw-kerf "
                        "disturbed markers). 0 keeps them.")
    p.add_argument("--cmap", default="cool",
                   help="Colormap for the Δt coloring (avoid maps with black so the "
                        "black pre-slope reference stands out).")
