...
class Video_util:
    ...
    def _apply_affine(M, x, y):
        # Map point (x, y) through a 2x3 affine transform M
    ...
    def _camera_transform(self, prev_pts, next_pts):
        '''
        Estimate the camera (background) motion between two frames.
        Fits a partial-affine transform (translation + rotation + uniform scale)
        from the background points with RANSAC, falls back to median translation
        when too few points are available, and to identity when there are none.
        Returns (2x3 transform mapping prev->next, points used, method).
        '''
        n = 0 if prev_pts is None else len(prev_pts)
        if n >= 3:
            M, inliers = cv2.estimateAffinePartial2D(prev_pts, next_pts,
                                                     method=cv2.RANSAC,
    ...
    def track(self, draw_speeds=True, stabilize=True):
        ...
        # Background features for camera-motion compensation. Track points outside
        # the ROI (dilated, to avoid edge bleed) to estimate the camera's own
        # motion, which is later subtracted from the ROI motion.
        bg_mask = None
        bg_p0 = None
        bg_params = dict(self.feature_params, maxCorners=200)
        if stabilize:
            roi_bin = (mask_gray > 0).astype('uint8') * 255
            roi_dilated = cv2.dilate(roi_bin, np.ones((15, 15), np.uint8))
        ...
        # Create DataFrame to store motion between frames (raw, plus corrected
        # columns and camera diagnostics when stabilizing)
        cols = []
        for i in range(self.feature_params['maxCorners']):
            cols.append('p' + str(i) + '_x')
            cols.append('p' + str(i) + '_y')
            if stabilize:
                cols.append('p' + str(i) + '_x_corr')
                cols.append('p' + str(i) + '_y_corr')
        if stabilize:
        ...
        for idx in tqdm(range(start, end)):
            ...
            # Estimate camera motion from background points
            cam_M, n_bg, cam_method = None, 0, 'none'
            if stabilize and bg_p0 is not None and len(bg_p0) > 0:
                bg_p1, bg_st, bg_err = cv2.calcOpticalFlowPyrLK(old_gray, frame_gray, bg_p0, None, **self.lk_params)
            ...
            for i,(new,old) in enumerate(zip(good_new,good_old)):
                ...
                # Subtract camera motion: where would a static point at (c, d) land
                # under the estimated camera transform? The residual is the object's
                # true motion relative to the scene.
                if cam_M is not None:
                    ex, ey = self._apply_affine(cam_M, c, d)
                    corr_dx = (a-ex)*self.pix_width
                    corr_dy = (b-ey)*self.pix_heigth
                    d_motion['p'+str(i)+'_x_corr']=corr_dx
                    d_motion['p'+str(i)+'_y_corr']=corr_dy
            ...
            if cam_M is not None:
                d_motion['bg_points'] = n_bg
                d_motion['cam_dx'] = float(np.median(cdxdt) - np.median(dxdt)) / self.fps if dxdt else 0.0
                d_motion['cam_dy'] = float(np.median(cdydt) - np.median(dydt)) / self.fps if dydt else 0.0

            motion = motion.append(d_motion, ignore_index=True)
            img = cv2.add(frame,mask)
            # When stabilizing, report the corrected speed; otherwise the raw speed
            use_corr = cam_M is not None and len(cdxdt) > 0
            mdx = np.median(cdxdt if use_corr else dxdt) if (cdxdt or dxdt) else 0
            mdy = np.median(cdydt if use_corr else dydt) if (cdydt or dydt) else 0
            dxdt = 0 if round(mdx, 2)==0 else round(mdx, 2)
            dydt = 0 if round(mdy, 2)==0 else round(mdy, 2)
            ...
            if stabilize:
                text_y3 = int(0.16 * height)
                conf_color = (0, 200, 0) if n_bg >= 10 else (0, 165, 255)
                img = cv2.putText(img, "bg pts: {0} ({1})".format(n_bg, cam_method), (text_x, text_y3), font, 0.7, conf_color, 1, lineType = cv2.LINE_AA)
